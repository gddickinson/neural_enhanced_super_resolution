...
class ImageViewer(QWidget):
    ...
    def update_display(self):
        """Update the image displays with current zoom level."""
...
class EnhancementSettings(QWidget):
    """Widget for configuring enhancement settings."""
    ...
    def init_ui(self):
        ...
        basic_layout.addWidget(self.save_intermediate)
        ...
        self.device.addItems(["CUDA (GPU)", "CPU"])
        ...
        advanced_layout.addWidget(self.preserve_details)
        advanced_layout.addWidget(self.adaptive_sharpen)
    ...
    def load_preset(self):
        ...
        # Custom presets would load from settings or files
...
class NESRApplication(QMainWindow):
    ...
    def init_ui(self):
        ...
        settings_tabs.addTab(self.enhancement_settings, "Enhancement")
    ...
    def start_enhancement(self):
        ...
        try:
            ...
            # Get settings
            config = self.enhancement_settings.get_settings()
            prompt = self.enhancement_settings.get_prompt()
            
            # Determine device
            device = "cuda" if self.enhancement_settings.device.currentText().startswith("CUDA") else "cpu"
            ...
            if self.degraded_image is not None:
                ...
                cv2.imwrite(tmp_path, cv2.cvtColor(self.degraded_image, cv2.COLOR_RGB2BGR))
